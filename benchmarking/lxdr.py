        ...
        if self.type == 'neural':
            ...
            if not self.use_shap:
                ...
                if self.shap == "kernel":
                    ...
                    _coef_per_component = lxdr_shap_grad(neighbours[0] - self.mean, model, reduced_dimensions, neighbours - self.mean)
                elif self.shap == "samp":
                    _coef_per_component = lxdr_shap_samp(neighbours[0] - self.mean, model, reduced_dimensions, neighbours - self.mean)
                else:
                    print("typo")
    ...
    def visualise_weights(self,
        ...
        Args:
            ...
            normalised: if True, data will be divided with the absolute max 
                value of the data.
        """
        if self.model.embedding_ is not None:
            original_coef_ = self.model.embedding_[dimension]
        else:
            print(
...
def lxdr_shap_grad(sample, model, reduced_dimensions, neighbours):
    shap_explanation = []
    for reduced_dimension in range(reduced_dimensions):
        def model_dimension(instance):
            return model.predict(instance)[:,reduced_dimension]
        explainer = shap.GradientExplainer(model_dimension, neighbours)
        shap_explanation.append(explainer.shap_values(np.array(sample)))
    return shap_explanation

def lxdr_shap_samp(sample, model, reduced_dimensions, neighbours):
    shap_explanation = []
    for reduced_dimension in range(reduced_dimensions):
        def model_dimension(instance):
            return model.predict(instance)[:,reduced_dimension]
        explainer = shap.SamplingExplainer(model_dimension, neighbours)
        shap_explanation.append(explainer.shap_values(np.array(sample)))
    return shap_explanation
