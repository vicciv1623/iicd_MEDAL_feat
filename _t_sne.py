...
def _joint_probabilities(distances, desired_perplexity, verbose):
    ...
    conditional_P, scales = _utils._binary_search_perplexity(
        distances, desired_perplexity, verbose
    )
    P = conditional_P + conditional_P.T
    sum_P = np.maximum(np.sum(P), MACHINE_EPSILON)
    P = np.maximum(squareform(P) / sum_P, MACHINE_EPSILON)
    return P, scales, conditional_P
...
def _gradient_descent(
    ...
    for i in range(it, max_iter):
        ...
        error, grad, Q = objective(p, *args, **kwargs)
...
class TSNE(ClassNamePrefixFeaturesOutMixin, TransformerMixin, BaseEstimator):
    ...
    def _fit(self, X, skip_num_points=0):
        ...
        if self.method == "exact":
            ...
            P, scales, cond_P = _joint_probabilities(distances, self.perplexity, self.verbose)
    ...
    def _tsne(
        ...
        params, kl_divergence, it, Q = _gradient_descent(obj_func, params, **opt_args)
        if self.verbose:
            print(
